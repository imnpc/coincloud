...
class CreateSystemWalletLogsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('system_wallet_logs', function (Blueprint $table) {
            ...
            $table->decimal('old_team_a', 16, 5)->comment('原有分红池A');
            $table->decimal('old_team_b', 16, 5)->comment('原有分红池B');
            $table->decimal('old_team_c', 16, 5)->comment('原有分红池C');
            $table->decimal('old_risk', 16, 5)->comment('原有风控账户');
            $table->decimal('old_commission_balance', 16, 5)->comment('原有推荐');
